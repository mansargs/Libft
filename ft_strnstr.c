...
void check_ft_strnstr(const char *str, const char *to_find, size_t len, const char *expected) {
    char *result_ft = ft_strnstr(str, to_find, len);
    
    if ((result_ft == expected) || (result_ft && expected && strcmp(result_ft, expected) == 0)) {
        printf("PASSED: str: \"%s\", to_find: \"%s\", len: %zu\n", str, to_find, len);
        printf("Expected: %s, Got: %s\n\n", expected ? expected : "NULL", result_ft ? result_ft : "NULL");
    } else {
...
int main() {
    ...
    check_ft_strnstr("a very long string with needle right at the end needle", "needle", 45, NULL);

    // 16. Complex pattern with overlapping sequence in the haystack
    check_ft_strnstr("abababababababab", "ababab", 12, "abababababababab");

    // 17. Needle found multiple times within haystack, should return first occurrence within len
    ...
    check_ft_strnstr("ab\001\002\003\004cd", "\001\002\003", 6, "\001\002\003\004cd");
    return 0;
}
*/